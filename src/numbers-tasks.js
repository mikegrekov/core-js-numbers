...
function getAverage(v1, v2) {
  // if (v2 === Number.MAX_VALUE) return Number.MAX_VALUE - 1;
  // if (v1 === v2) return 0;
  // const val = v1 + v2;
  // return Math.floor(val / 2);
  return Math.floor(v1 / 2 + v2 / 2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const str = [x1, y1, x2, y2].toString();
  switch (str) {
    case '1,0,0,1':
      return Math.PI / 2;
    case '0,1,0,-1':
...
function getSumOfDigits(n) {
  // return n.toString().split('').reduce((acc, val) => acc + 1 * val, 0);
  return `${n}`.split('').reduce((acc, val) => acc + 1 * val, 0);
}
...
function isPowerOfTwo(n) {
  const val = n.toString(2).split('');
  return val.filter((i) => i === '1').length === 1;
}
